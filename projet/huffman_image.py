...
def wrtie_file(dico, compressed_file, file):
    ...
    with open(file+"_compressed.pgm", 'w') as file:
...
def write_deco(ficher, string):
    ...
    with open(ficher+"_decompression.pgm", "w") as ft:
...
decompression_avec_dico("projet/Logo_Python_compressed.pgm")

fin_temps = time.time()

temps_d_execution = fin_temps - debut_temps

print(temps_d_execution)



#decompression_avec_dico("franck.pgm")



#compare_size("", "adn_compressed.txt")
...
# si on sauvegarde l'arbre decompression_avec_arbre("projet/adn_compressed.txt", arbre) --> decompression + creation du ficher
